...
class NetVLADFeatureExtractor:
    def __init__(self):
        self.config = os.path.join(PATCHNETVLAD_ROOT_DIR, 'configs/netvlad_extract.ini')
...
class NetVLAD(BaseTechnique, NetVLADFeatureExtractor):
    ...
    def compute_query_desc(self, query_images: np.ndarray) -> dict:
        query_desc = self.compute_features(query_images, disable_pbar=True)
        return {"query_descriptors": query_desc}


    def compute_map_desc(self, map_images: np.ndarray) -> dict:
        map_desc = self.compute_features(map_desc, disable_pbar=True)
